...
use cubemoma::{BigField, NttPrecompute};

const N: usize = 7;
type Fp = BigField<N>;

// p = 2^448 - 2^224 - 1 = "0x FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF FFFFFFFFEFFFFFFF FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF"
...
fn bench_cpu_fp_ops(c: &mut Criterion) {
    ...
    c.bench_function("fp_mul_mod", |bencher| {
        bencher.iter(|| black_box(a.mul_mod(&b, &modulus, mu)));
    });

    c.bench_function("fp_square_mod", |bencher| {
        bencher.iter(|| black_box(a.square_mod(&modulus, mu)));
    });
    ...
    c.bench_function("fp_repeated_mul_mod", |bencher| {
        bencher.iter(|| {
            let mut res = a;
            for _ in 0..1000 {
                res = black_box(res.mul_mod(&b, &modulus, mu));
            }
            res
        });
    });
    ...
    let precompute = NttPrecompute::<7>::new(&omega, n, &modulus, mu);
    c.bench_function("fp_ntt", |bencher| {
        bencher.iter(|| {
            black_box(Fp::ntt(&mut input, &precompute, &modulus, &mu));
        });
    });

    // 긴 연산 2: 모듈러 곱셈 역원
    c.bench_function("fp_inv_mod", |bencher| {
        bencher.iter(|| black_box(Fp::from_limb(7u64).inv_mod(&modulus, mu).unwrap()));
    });

    // 긴 연산 3: 유한체 내에서의 제곱근
    c.bench_function("fp_sqrt_mod", |bencher| {
        bencher.iter(|| black_box(Fp::from_limb(49u64).sqrt_mod(&modulus, mu).unwrap()));
    });
}
